...
class Classica(torch.nn.Module):
    def __init__(self, pmt_N, **kwargs):
        ...
        self.dimmWgt0 = torch.nn.Parameter(torch.ones(1))
        #self.dimmWgt1 = torch.nn.Parameter(torch.zeros(1))
        #self.wgtAngl = torch.nn.Parameter(torch.randn(pmt_N, self.optModelOrder))

        #self.fc1 = torch.nn.Sequential(
        #    torch.nn.Linear(2*pmt_N, 4096),
    ...
    def forward(self, pmt_q, pmt_t, pmt_pos, pmt_dir, vtxPos):
        ...
        dimmDist = torch.sigmoid(self.dimmWgt0)/dr2 #*torch.exp(dr*self.dimmWgt1.view(1,1))
        ...
        #dimmAngl = (cosTs*self.wgtAngl.view(1, *self.wgtAngl.shape)).sum(dim=-1)
